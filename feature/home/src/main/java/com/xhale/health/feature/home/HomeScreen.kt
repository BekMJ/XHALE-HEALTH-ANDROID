...
@Composable
fun HomeRoute(viewModel: HomeViewModel, onNavigateToBreath: () -> Unit) {
        ...
        Box(
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                if (state.devices.isEmpty() && !state.isScanning) {
                    Text("No devices found. Tap 'Start Scan' to discover devices.", color = Color.White)
                } else {
                    LazyColumn {
                            ...
                            Card(modifier = Modifier.padding(vertical = 4.dp).clickable { onConnect(device.deviceId) }) {
                                Column(modifier = Modifier.padding(8.dp)) {
                                    ...
                                    if (state.connectedDeviceId == device.deviceId) {
                                        ...
                                        Row { Text("CO: ${state.coPpm?.let { String.format("%.2f", it) } ?: "--"} ppm") ; Spacer(Modifier.size(12.dp)); Text("Temp: ${state.temperatureC?.let { String.format("%.2f", it) } ?: "--"} °C") }
                                    }
                                }
                            }
                        }
